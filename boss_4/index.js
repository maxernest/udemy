...
async function retrieveData(){
	return new Promise(async function(resolve){
		var blogs = await createConnection();
		var posts = await blogs.find();
		resolve(posts);
	})
}
...
async function getPost(id){
	return new Promise(async function(resolve){
		var blogs = await createConnection();
		var post = await blogs.findById(id).exec();
		resolve(post);
	})
}
...
app.post("/compose", async function(req, res){
	await addPost(req.body.title, req.body.content);
	res.redirect("/");
})
